...
async function uploadToImageKit(
	...
	try {
		...
		console.error(
			`Error uploading ${fileName} to ImageKit:`,
			error
		)
		throw error
	}
...
async function seed() {
	...
	try {
		for (const book of dummyBooks) {
			...
			console.log(`[IMK] Uploaded cover for ${book.title}`)
			...
			console.log(`[IMk] Uploaded video for ${book.title}`)
			...
			console.log(`[DB] Added book: ${book.title}`)
		}
